...
public class Hub implements IHub {
    ...
    public void readTAGNFC(){
        try{
            Intent intent = new Intent(ACTION_SCAN);
            intent.putExtra("SCAN_MODE","TAG_NFC_MODE");
            mContext.startActivityForResult(intent,0);
        }catch (ActivityNotFoundException anfe){
            //Uri marketUri = Uri.parse("market://details?id=com.tantranshanfc.mainpart");
            Uri marketUri = Uri.parse("market://details?id=mobile.fxmanager");
            Intent marketIntent = new Intent(Intent.ACTION_VIEW,marketUri);
            mContext.startActivity(marketIntent);
        }
    }
